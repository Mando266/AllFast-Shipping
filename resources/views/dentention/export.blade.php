                    ...
                    <nav class="breadcrumb-two" aria-label="breadcrumb">
                        <ol class="breadcrumb">
                            ...
                            <li class="breadcrumb-item active"><a
                                    href="javascript:void(0);">{{ trans('menu.exp_dentention_cal') }}</a>
                            </li>
                    ...
                    <form action="{{ route('export_dentention_calculation') }}">
                        ...
                        <div class="form-row">
                            <div class="form-group col-md-3">
                                ...
                                <select class="selectpicker form-control" name="booking_type"
                                    title="{{ trans('forms.select') }}" required>
                                    <option value="full" selected>{{ trans('menu.full') }}</option>
                                    <option value="empty" disabled>{{ trans('menu.empty') }}</option>
                                </select>
                            </div>

                            <div class="form-group col-md-3">
                                <label for="shipment_type">{{ trans('menu.shipment_type') }}</label>
                                <select class="selectpicker form-control" data-live-search="true" id="shipment_type"
                                    name="shipment_type" title="{{ trans('forms.select') }}" required>
        ...
        <script>
            $('#shipment_type').change(function (e) {
                ...
                if (shipment_type == 'Import') {
                    targetTextFromCode = 'DCHF';
                    targetTextToCode = ['ALL', 'RCVC', 'RSTR'];
                }
                from_code.append(
                    `<option value=${targetTextFromCode} selected }>${targetTextFromCode} </option>`);
                $.each(targetTextToCode, function (index, value) {
                    to_code.append(
